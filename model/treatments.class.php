...
class ManagementBddTreatments extends crud {
	...
	// UPDATE
    public function update_med(Treatments $treatment){
    ...
    public function select_treatment_for_1ppl($id){
        ...
        $query = $this->getDb()->prepare('SELECT * FROM treatments
            WHERE t_aid = :t_aid AND t_del = 0 AND t_validate > 0
			ORDER BY t_tag');
	...
	// DELETE
	public function delete_treatment_for_1ppl($id){
		$query = $this->getDb()->prepare('DELETE FROM treatments
			WHERE t_aid = :t_aid');
