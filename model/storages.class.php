...
class ManagementBddStorages extends crud {
    ...
    public function update_storage(Storages $storage){
        $query = $this->getDb()->prepare('UPDATE storages 
            SET s_id = :s_uid, s_cis = :s_cis, s_cip = :s_cip, s_bundle = :s_bundle, s_dosage = :s_dosage, s_quantity = :s_quantity, s_date = :s_date
            WHERE s_id = :s_id');
    ...
    public function delete_storage_with_uid($id){
		$query = $this->getDb()->prepare('DELETE FROM storages
			  WHERE s_uid = :s_uid');
	...
	public function delete_storage_with_sid($id){
		$query = $this->getDb()->prepare('DELETE FROM storages
			WHERE s_id = :s_id');

		$query->bindValue(':s_id',$id, PDO::PARAM_INT);
