...
class ManagementBddUsers extends crud {
    ...
    // UPDATE
    public function update_user_by_id(Users $users,$pass2){
		if(preg_match("/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,20}$/", $users->getu_password())){
			if($users->getu_password() == $pass2){
				$cryptpass = password_hash($users->getu_password(), PASSWORD_DEFAULT);
						
				$query = $this->getDb()->prepare('UPDATE users 
					SET u_password = :u_password, u_mail = :u_mail
					WHERE u_id = :u_id');
					
				$query->bindValue(':u_id', $users->getu_id(), PDO::PARAM_INT);		
				$query->bindValue(':u_password', $cryptpass, PDO::PARAM_STR);
	...
	//DELETE
	public function delete_user_with_user_id($id){
		$query = $this->getDb()->prepare('DELETE FROM users
			WHERE u_id = :u_id');
			
		$query->bindValue(':u_id',$id, PDO::PARAM_INT);
