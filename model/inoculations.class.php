...
class ManagementBddInoculations extends crud {
    ...
    public function update_inoculation(Inoculations $ino){
		...
		$query->bindValue(':i_id', $ino->geti_id(), PDO::PARAM_INT);
		$query->bindValue(':i_aid', $ino->geti_aid(), PDO::PARAM_INT);			
        $query->bindValue(':i_vid', $ino->geti_vid(), PDO::PARAM_INT);
        $query->bindValue(':i_date', $ino->geti_date(), PDO::PARAM_STR);
	...
	public function update_inoculation_bis(Inoculations $ino){
        $query = $this->getDb()->prepare('UPDATE inoculations 
            SET i_date = :i_date 
            WHERE i_id = :i_id');
	...
	public function select_inoculation_for_1ppl_join($id){
        $resultat =[];
        
        $query = $this->getDb()->prepare('SELECT i_id,v_name, i_date, v_disease, v_type, v_date FROM inoculations
            JOIN vaccines ON i_vid = v_id
			WHERE i_aid = :i_aid');
	...
	//DELETE
    public function delete_inoculation_for_1ppl($id){
		$query = $this->getDb()->prepare('DELETE FROM inoculations
			WHERE i_aid = :i_aid');
